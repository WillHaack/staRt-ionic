...
sessionStatsService.factory('SessionStatsService', function($rootScope, $localForage, $http, FirebaseService, NotifyingService, ProfileService, $state)
	...
	function _updateHighscores(msg, highscores) {
	    ProfileService.getCurrentProfile().then(function (profile) {
		if (profile) {
		    var changelist = []; // not sure what this is

		    _incrementProfileStat(profile, 'highscores', highscores, changelist);
		    _updateProfileStat(profile, 'highscores', highscores, changelist);
                    ProfileService.saveProfile(profile);
                    _notifyChanges(profile, currentProfileStats, changelist);
		}
	    });
