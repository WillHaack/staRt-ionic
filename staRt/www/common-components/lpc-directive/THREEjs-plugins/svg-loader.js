/**
  This is an imperfect THREE.js plugin.

  What it does:
  1.) uses the THREE fileloader to load svg files
  2.) attempts to parse the svg nodes (style & shapes)
  3.) creates basic materials, based on the parsed style node
  4.) construct a THREE shape, based on each path or shape primitive
  5.) returns a shape mesh of each svg node
  - hc
